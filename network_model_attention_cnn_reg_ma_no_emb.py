...
class GTrXLNet(RecurrentNetwork):
    ...
    def __init__(self,
        ...
        self.max_seq_len = model_config["max_seq_len"]
        #self.obs_dim = observation_space.shape[0]
        self.obs_dim = observation_space.original_space.spaces["states"].shape[0]
        ...
        x = tf.keras.layers.Reshape((-1,5,186))(input_layer)
        ...
        if num_outputs is not None:
            ...
            self._logits = tf.keras.layers.Dense(self.num_outputs, activation=None, name="logits", kernel_initializer=kernel_initializer)(E_out)
            
            kernel_initializer = tf.keras.initializers.Orthogonal(gain=1)
            values_out = tf.keras.layers.Dense(1, activation=None, name="values", kernel_initializer=kernel_initializer)(E_out)
    ...
    def forward(self, input_dict, state: List[TensorType],
        ...
        observations = input_dict[SampleBatch.OBS]["states"]
